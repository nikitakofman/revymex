...
const DraggedNode: React.FC<DraggedNodeProps> = ({ node, content, offset }) => {
  ...
  if (dragPositions) {
    ...
    finalLeft = transform.x + dragPositions.x * transform.scale;
    finalTop = transform.y + dragPositions.y * transform.scale;
  } else {
    ...
    // Special handling for absolutely positioned nodes in frames
    let rawLeft: number;
    let rawTop: number;
    ...
    if (dragSource === "absolute-in-frame" || isAbsoluteInFrame(node)) {
      ...
      rawTop =
        baseRect.top - containerRect.top - offset.mouseY * transform.scale;
    }
    ...
    // add rotation offset
    finalLeft = rawLeft + offsetX * transform.scale;
    finalTop = rawTop + offsetY * transform.scale;
