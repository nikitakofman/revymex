          ...
          .map((verticalAlign, vIndex) =>
            ["start", "center", "end"].map((horizontalAlign, hIndex) => {
              ...
              return (
                ...
                `}
                  onClick={() => {
                    // If an advanced distribution is selected, preserve it when changing alignment
                    const newJustify = isAdvancedDistribution
                      ? distribution
                      : getFlexValue(justifyValue);
                    onChange({
                      justify: newJustify,
                ...
                >
                  {/* Show blue bars like in the screenshot */}
...
export default function LayoutTool() {
  ...
  const handleAlignmentGridChange = ({ justify, align }) => {
    // Reset distribution to "stack" mode if it was an advanced option
    const isAdvancedDistribution = [
      "space-between",
      "space-around",
      "space-evenly",
    ...
    if (isAdvancedDistribution) {
      // If we were in advanced mode and user clicks grid, switch to "stack" mode
      setDistribution("stack");
    } else {
      // In stack mode, update justify content normally
    ...
    setNodeStyle(
      {
        justifyContent: isAdvancedDistribution ? justify : justify,
        alignItems: align,
      },
      undefined,
      true
